...
Among the Egyptian antiquities in the Detroit Institute of Arts (DIA) is a Roman-period funerary portrait ([fig. 15.1A](#fig-15-1-a)).[^1] Little is known about this portrait’s archaeological provenience. Records at the DIA indicate only that the object was given to the museum in 1925[^2] by Julius Haass (1869–1931),[^3] who, at an unknown time, had purchased it from the art dealer Dikran Kelekian (1867–1951).[^4]

{% figuregroup '2' 'fig-15-1-a, fig-15-1-b' %}

Despite the lack of archeological information on the DIA panel, the similarities in style and provenance with another funerary portrait now housed in the collection of the Walters Art Museum in Baltimore ([fig. 15.1B](#fig-15-1-b))[^5] have been known since David Lowell Thompson’s 1972 thesis.[^6] Most notable are the nearly identical high-relief coin pendant necklaces ([figs. 15.2A](#fig-15-2-a) and [B](#fig-15-2-b)), which (to date) are unique to these two portraits. Both portraits are believed to have originated from Antinoöpolis[^7] and their sale to American collectors was facilitated by the Kelekian Gallery in both instances.[^8]

{% figuregroup '2' 'fig-15-2-a, fig-15-2-b' %}
...
The Detroit portrait was examined by close visual observation, using primarily non-destructive instrumentation relying heavily on point analysis with visible near-infrared fiber optics reflectance spectroscopy (vis-NIR FORS), X-ray fluorescence (XRF) spectroscopy, and various imaging techniques. Samples were removed from the edge of the panel to yield further information about the binding media and pigment components of select colors. For technical information on the corresponding panel, we relied on the extensive research that was undertaken by the Walters Museum’s Department of Conservation and Technical Research.[^9]
...
[^6]: {% cite 'Thompson 1972' '34–36' %}.

[^7]: Based mostly on the distinct “stepped” contour of the wood panel. For further information, see {% cite 'Thompson 1972' %}.
...
[^9]: {% cite 'Radpour et al. 2022' %}; <https://doi.org/10.1186/s40494-021-00639-5>; File 32.4, Department of Conservation and Technical Research, Walters Art Museum, Baltimore; APPEAR database entry for Walters Art Museum, Panel Portrait of a Woman, 32.4, accessed December 28, 2022, <https://www.appeardatabase.org/portrait/531/>; {% cite 'Griswold 2014' %}.

[^10]: {% cite 'Cartwright 2022' %}.

[^11]: {% cite 'Cartwright 2020' '16–23' %}.

[^12]: {% cite 'Spaabæk 2012' '66–67' %}; review of data available on APPEAR database, accessed December 28, 2022, <https://www.appeardatabase.org>: Of the fourteen panels here believed to be from Antinoöpolis (with wood identifications), 64% are a wood other than linden (five sycomore fig, two cedar of Lebanon, one tamarisk, one beech).

[^13]: Review of data available on APPEAR database, accessed December 28, 2022, <https://www.appeardatabase.org>.

[^14]: {% cite 'Spaabæk 2012' '67' %}; {% cite 'Cartwright, Spaabæk, and Svoboda 2011' '57' %}.

[^15]: Fragments were analyzed with Fourier transform infrared spectroscopy (FTIR). The sample correlates with a diterpenoid resin, with a highest correlation to exudates from Pinaceae. See {% cite 'Tappert et al. 2011' '120–38' %}.
...
[^24]: Identification based on SEM/EDS analysis of a sample of the ground in cross section. In point analysis, red areas were consistent with iron oxide and silica and calcite grains were also noted. A significant portion of the ground was identified as clay based on the characteristic 2:1 ratio of Si:Al with varying amounts of K/Mg/Na. Small particles with calcium and phosphorus are also noted, indicating a geological apatite/phosphate mineral or possibly bone black.

[^25]: {% cite 'Radpour et. al. 2022' '8' %}.

[^26]: For example: review of data available on APPEAR database, accessed December 28, 2022, <https://www.appeardatabase.org>; {% cite 'Ebbinghaus et. al 2022C' %}; and {% cite 'Park et al. 2019' %}.

[^27]: The use of Egyptian blue was inferred through visible-induced NIR luminescence alone. For further information on the Walters identification see {% cite 'Radpour et al. 2022' %}; {% cite 'Verri et al 2010' %}; and {% cite 'Ganio et al. 2015' %}.

[^28]: {% cite 'Thiboutot 2020' '48–50' %}; {% cite 'Ganio et al. 2015' '9' %}.

[^29]: Confirmed as a plant-based red lake through vis-NIR FORS; {% cite 'Newman and Gates 2020' '24–30' %}. Indigo was deduced through vis-NIR FORS by visible spectra with a sharp increase in reflectance at ~690 nm, with an inflection point at ~725 nm, consistent with indigo. See {% cite 'Ichimiya 2021' '97–99' %}. The depiction of a white and purple bichromatic shawl is most likely not authentic to the painting. Because of the large loss over the proper right shoulder, the second purple stripe on the white tunic was misinterpreted as part of the scarf during a past restoration. The scarf most likely just covered the proper left shoulder. For further information see {% cite 'Bisulca et al. 2021' '12–14' %}.

[^30]: Identification through vis-NIR FORS and FTIR for DIA painting (see APPEAR database). For wax identification in the Walters painting see {% cite 'Radpour et al. 2022' '8–9' %}; {% cite 'Ramer 1979' '6–7' %}; and {% cite 'Mazurek 2020' '142–47' %}.

[^31]: Egyptian, probably from Antinoöpolis, Portrait of a Woman, ca. 130–50 CE. Encaustic on native sycomore fig, 35.3 x 22.5 x 2 cm (14 x 9 x 7/8 in.). Harvard Art Museums/Arthur M. Sackler Museum, 1923.60. {% cite 'Ebbinghaus et al. 2022B' %}.

[^32]: For a more detailed description of the painting materials and techniques used in the Walters funerary portrait see {% cite 'Radpour et. al. 2022' '6–14' %}. All technical information listed here is based on the findings published in this article. For further information on the making of the Detroit portrait see {% cite 'Bisulca et al. 2021' '6–19' %}.

[^33]: Review of data available on APPEAR database. See also {% cite 'Thompson 1976A' '115–19' %}.

[^34]: {% cite 'Bruhn 1993' '30–32' %}.

[^35]: The wood from the DIA panel was radiocarbon dated at the Accelerator Mass Spectrometry Laboratory, University of Arizona. The results are a calendar date range of 132 calCE to 335 calCE (95%), 214 calCE to 315 calCE (68%). See {% cite 'Bisulca et al. 2021' '11' %}.

[^36]: Shroud, Egyptian (Antinoöpolis), 200–299 CE. Encaustic on linen, 115 x 62 cm (45 1/4 x 24 1/2 in.). Musée du Louvre, Paris, AF 6486. Painted Shroud, Mummy Portrait, Egyptian (Antinoöpolis), 200–249 CE. Tempera and encaustic with gilt stucco on linen, 81 x 45 cm (32 x 17 3/4 in.). Musée du Louvre, Paris, AF 6487.

[^37]: Portrait of a Woman, Egyptian (Antinoöpolis), 130–161 CE. Encaustic on wood with gilt stucco, 44.45 x 17.15 cm (17 1/2 x 6 3/4 in.). The Nelson-Atkins Museum of Art, Kansas City, Purchase: William Rockhill Nelson Trust, 37-40.

[^38]: The gesso is consistent with calcium carbonate in a proteinaceous binder based on vis-NIR FORS. Gessos made with calcium carbonate appear to be common in ancient Egyptian artefacts, possibly more so than gypsum. See {% cite 'Hatchfield and Newman 1991' '34–38' %}.

[^39]: {% cite 'Radpour et. al. 2022' '9' %}.

[^40]: {% cite 'Radpour et. al. 2022' '8' %}.

[^41]: {% cite 'Bruhn 1993' '10–16' %}.

[^42]: {% cite 'Bruhn 1993' '32' %}. Bruhn notes the modeling of the necklace could also indicate an embossed gold neck ring.

[^43]: This material was removed during a treatment in the 1970s. See Walters Conservation File 32.4, Walters APPEAR database entry.

[^44]: Today, the Detroit necklace is held in place with a clear modern glue, applied likely during a 1970s repair after an incident. No conservation record has been preserved. See Registration file for 25.2 (also see note 2). A note in the conservation file mentions that a portion of the raised stucco necklace was replaced in 1947 by conservator William Suhr. See File 25.2, Conservation Department, Detroit Institute of Arts.

[^45]: For the historic image, see <https://art.thewalters.org/detail/30478/mummy-portrait-of-a-woman-from-fayum-egypt/>.

[^46]: For more on the restoration and the resulting misrepresentation of the garment see {% cite 'Bisulca et al. 2021' '12–14' %}.

[^47]: {% cite 'Stein and Corcoran 2020' '128–31' %}.

[^48]: Henry Walters purchased altogether four Roman Egyptian funerary portraits from Kelekian in 1912. Walters Art Museum, Baltimore 32.4, 32.5, 32.6, 32.7.

[^49]: {% cite 'Cartwright 2022' %}.

[^50]: {% cite 'Riggs 2012' '664' %}.

[^51]: {% cite 'Kersel 2021' '263–64' %}; and {% cite 'Hopkins, Costello, and Davis 2021' '1–25' %}.

[^52]: See also introductory text panels “Facing Forward” and “Statement from the Curators” to the exhibition *Funerary Portraits from Roman Egypt: Facing Forward*, held at the Harvard Art Museums, August 27–December 30, 2022.
